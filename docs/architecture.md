...
- **Audit:** хранит решение, причины, scores и версии без исходного текста.
...
При недоступности LLM circuit breaker отключает генерацию, но классификация и маршрутизация продолжают работать. Payment, account takeover, legal/security, prompt injection, слабый retrieval и низкая уверенность всегда означают `ESCALATE`.
